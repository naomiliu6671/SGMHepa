...
class SelfAttentionLayer(nn.Module):
    ...
    def forward(self, inputs, mask=None, mask_rnn=None):
        ...
        context, _ = self.self_attn(inputs_ln, inputs_ln, inputs_ln, mask=mask)
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, num_layers=3, d_model=128,
        ...
        if self.layer_type == 'self':
            #
            self.transformer = nn.ModuleList([SelfAttentionLayer(d_model, int(d_model / 16), d_model * 4, dropout)
